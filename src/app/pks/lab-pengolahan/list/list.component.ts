...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/PksLabPengolahan/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.pksLabPengolahan = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.pksLabPengolahan.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.pksLabPengolahanService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let pksLabPengolahan;
    this.pksLabPengolahanService.getById(id).subscribe(data => {
      pksLabPengolahan = data['data'];
