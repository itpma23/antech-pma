...
export class ListComponent implements OnInit {
  ...
  exportFiles(type) {
    ...
    this.exportAsService.save(this.exportAsConfig, 'cost').subscribe(() => { });
  }
  ...
  reportCostDetail() {
    ...
    this.accJurnalService.getLaporanCostStasiunDetail(dataSubmit).subscribe((res: any) => {
      //console.log(res);
  ...
  reportPemakaianBahanStasiun() {
    ...
    if (isNull (this.pemakaianBahanForm.controls['stasiun'].value)) {
      stasiun_id = null;
    } else {
      stasiun_id = this.pemakaianBahanForm.controls['stasiun'].value['id'] ? this.pemakaianBahanForm.controls['stasiun'].value['id'] : null;

    }
    let mill_id = this.pemakaianBahanForm.controls['mill'].value['id'] ? this.pemakaianBahanForm.controls['mill'].value['id'] : null;
    ...
    console.log(dataSubmit);
    this.accJurnalService.getLaporanPemakaianBahanStasiun(dataSubmit).subscribe((res: any) => {
      if (format_laporan == 'xls') {
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
        saveAs(blob, 'report_cost.xls')
