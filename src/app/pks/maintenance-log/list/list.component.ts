...
const MenuName = 'maintenance-log';
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/PksMaintenanceLog/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.pksMaintenanceLog = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.pksMaintenanceLog.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.pksMaintenanceLogService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let pksMaintenanceLog;
    this.pksMaintenanceLogService.getById(id).subscribe(data => {
      pksMaintenanceLog = data['data'];
