...
const MenuName = 'pks_harga_angkut';
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/PksHargaAngkut/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.pksHargaAngkut = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.pksHargaAngkut.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.pksHargaAngkutService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let pksHargaAngkut;
    this.pksHargaAngkutService.getById(id).subscribe(data => {
      pksHargaAngkut = data['data'];
