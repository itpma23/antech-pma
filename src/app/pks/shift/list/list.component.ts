...
export class ListComponent implements OnInit {
  ...
  constructor(
    ...
    private shiftService: PksShiftService,
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/pksShift/list', dataTablesParameters, {})
          .subscribe(resp => {
            this.shift = resp.data;
  ...
  exportFiles(type) {
    ...
    this.exportAsService.save(this.exportAsConfig, 'shift').subscribe(() => { });
  }
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.shiftService.delete(id).subscribe(data => {
        that.rerender();
        swal({
          title: 'Deleted!',
  ...
  edit(id: number) {
    let that = this;
    let shift;
    this.shiftService.getById(id).subscribe(data => {
      shift = data['data'];
      let modalConfig = {
        animated: true,
        keyboard: true,
  ...
  // read data
  detail(id: number) {
    this.router.navigate(['shift/detail', id.toString(),{previousUrl: this.router.url}]);
