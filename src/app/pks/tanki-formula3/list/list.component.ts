...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/PksTankiFormula3/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.pksTankiFormula3 = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.pksTankiFormula3.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.pksTankiFormula3Service.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let pksTankiFormula3;
    this.pksTankiFormula3Service.getById(id).subscribe(data => {
      pksTankiFormula3 = data['data'];
