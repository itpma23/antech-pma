...
const MenuName = 'pks_produksi_harian';
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/PksProduksiHarian/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.pksProduksiHarian = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.pksProduksiHarian.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.pksProduksiHarianService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
  ...
  edit(id: number) {
    let that = this;
    let pksProduksiHarian;
    this.pksProduksiHarianService.getById(id).subscribe(data => {
      pksProduksiHarian = data['data'];
      ...
      let modalConfig = {
        ...
        // class: "modal-lg ",
        initialState: {
          pksProduksiHarian: pksProduksiHarian
        }
      };
  ...
  prosesProduksiHarian() {
    ...
    this.bsModalRef = this.bsModalService.show(ProsesComponent, modalConfig);
    this.bsModalRef.content.event.subscribe(result => {
      if (result == 'OK') {
        // let t= $('#datatables').DataTable().ajax.reload();
        // t.draw();
