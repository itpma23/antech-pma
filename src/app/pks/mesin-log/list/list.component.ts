...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/PksMesinLog/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {

            console.log(resp);

            this.pksMesinLog = resp.data;
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.pksMesinLogService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let pksMesinLog;
    this.pksMesinLogService.getById(id).subscribe(data => {
      pksMesinLog = data['data'];
