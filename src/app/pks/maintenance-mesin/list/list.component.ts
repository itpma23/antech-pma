...
const MenuName = 'maintenance-mesin';
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      columns: [
        ...
        {'data': 'kete', },
        {'data': 'hm_km', },
        {'data': 'hm_km_maintenance', }

      ],

      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/PksMaintenanceMesin/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.pksMaintenanceMesin = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.pksMaintenanceMesin.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.pksMaintenanceMesinService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let pksMaintenanceMesin;
    this.pksMaintenanceMesinService.getById(id).subscribe(data => {
      pksMaintenanceMesin = data['data'];
