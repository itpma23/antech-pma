...
const MenuName = 'sounding-kernel';
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/PksSoundingKernel/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.pksSoundingKernel = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.pksSoundingKernel.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.pksSoundingKernelService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let pksSoundingKernel;
    this.pksSoundingKernelService.getById(id).subscribe(data => {
      pksSoundingKernel = data['data'];
