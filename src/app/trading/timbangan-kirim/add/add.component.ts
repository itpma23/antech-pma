...
export class AddComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.entryForm = this.builder.group({
      ...
      tanki_id: new FormControl([]),
      transportir_id: new FormControl([]),
  ...
  this.entryForm.get('tara_kirim').valueChanges.subscribe(() => {
    this.generateNetto();
  });

  this.entryForm.get('bruto_kirim').valueChanges.subscribe(() => {
    this.generateNetto();
  });
...
generateNetto() {
  ...
  let tara = Number(this.entryForm.get('tara_kirim').value) || 0;
  let bruto = Number(this.entryForm.get('bruto_kirim').value) || 0;
