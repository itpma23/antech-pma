...
currentContractFile: any = null;
  ...
  constructor(private builder: FormBuilder,
    ...
    this.entryForm = this.builder.group({
      ...
      upload_kontrak_manual: new FormControl(null),
...
formData.append('data', JSON.stringify(frmData));

if (this.selectedContractFile) {
  formData.append('userfile', this.selectedContractFile);
}

    //  return;
    this.PrcPoService.update(this.PrcPo.id, formData).subscribe(data => {
      if (data['status'] == 'OK') {
        console.log('ok');
        swal({
          title: 'Info!',
...
ngOnInit() {
  ...
  if (
    this.PrcPo &&
    this.PrcPo.upload_kontrak_manual
  ) {

    this.currentContractFile = {
      name: this.PrcPo.upload_kontrak_manual
    };
 ...
 onFileSelected(event: any): void {
  ...
  this.currentContractFile = file;

  this.entryForm.patchValue({
    upload_kontrak_manual: file
  });
