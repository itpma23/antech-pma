...
export class EditComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.entryForm.get('no_referensi').patchValue(this.accApInvoice.no_referensi);
  ...
  private loadSelect2(): void {
    ...
    let selectTermin;
    this.dataSelectTermin.forEach(a => {
      if (a.id == this.accApInvoice.termin_id) {
        selectTermin = a;
      }
    });
    this.entryForm.controls['termin_id'].patchValue(selectTermin);
    ...
    this.accApInvoiceService.getAllAkunKreditSalesInvoice().subscribe(x => {
      ...
      let selectedAkunKredit = null;

      x['data'].forEach(d => {

        this.dataSelectAkunKredit.push({
          id: d.acc_akun_id,
          text: d.kode_akun + ' - ' + d.nama_akun
        });

        if (String(this.accApInvoice.akun_kredit_id) === String(d.acc_akun_id)) {
      ...
      console.log('selected', selectedAkunKredit);

      if (selectedAkunKredit) {
        this.entryForm.get('acc_akun_id_kredit').patchValue(selectedAkunKredit);
      }
  ...
  onSubmit() {
    ...
    console.log(frmData);
    this.accApInvoiceService.update(this.accApInvoice.id, frmData).subscribe(data => {
      // console.log(data);
      if (data['status'] == 'OK') {
        console.log('ok');
        swal({
