...
import { TradingPrService } from 'src/app/shared/services/trading_pr.service';


@Component({
  moduleId: module.id,
  selector: 'pp-status-approval-cmp',
  styleUrls: ['pp_status_approval.component.css'],
  templateUrl: 'pp_status_approval.component.html'
})

export class TradingPpStatusApprovalComponent implements OnInit,AfterViewInit{
...
namaKaryawan5;
  ...
  constructor(private builder: FormBuilder,
    ...
    private PrcPpService: TradingPrService,
  ...
  ngAfterViewInit(): void {
    ...
    this.entryForm.controls['status'].patchValue(this.PrcPp.last_approve_position);

    this.entryForm.get('tanggal').patchValue(new Date(Date.parse(this.PrcPp.tanggal)));
    this.karyawanService.getById(this.PrcPp['user_approve1']).subscribe(k=>{

      this.namaKaryawan1= k['data']['nama'];
    })
    this.karyawanService.getById(this.PrcPp['user_approve2']).subscribe(k=>{

      this.namaKaryawan2= k['data']['nama'];
    })
    this.karyawanService.getById(this.PrcPp['user_approve3']).subscribe(k=>{

      this.namaKaryawan3= k['data']['nama'];
    })
    this.karyawanService.getById(this.PrcPp['user_approve4']).subscribe(k=>{

      this.namaKaryawan4= k['data']['nama'];
    })
    this.karyawanService.getById(this.PrcPp['user_approve5']).subscribe(k=>{
