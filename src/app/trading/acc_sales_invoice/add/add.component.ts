...
export class AddComponent implements OnInit, AfterViewInit {
  ...
  constructor(private builder: FormBuilder,
    ...
    private slsPerformaService: AccSalesPerformaService,
    ...
    this.entryForm = this.builder.group({
      ...
      mode_input: new FormControl('PROFORMA', Validators.required),
  ...
  private loadSelect2(): void {
    ...
    this.slsPerformaService.getPerforma().subscribe(res => {
      this.proformaList = res['data'];
      this.dataSelectProforma = this.proformaList.map(p => ({
        id: p.id,
        text: p.no_performa + ' | ' + p.tanggal + ' | ' + p.grand_total
      }));
    });
    ...
    this.slsPerformaService.getPerforma().subscribe(res => {
      ...
      this.dataSelectProforma = this.proformaList.map(p => ({
        id: p.id,
        text: p.no_performa + ' | ' + p.customer + ' | ' + p.grand_total
      }));
    });
  ...
  ngOnInit() {
    ...
    this.entryForm.get('jenis_invoice').valueChanges.subscribe(val => {
      ...
      console.log('jenis invoice changed: ' + jenis);
    ...
    this.entryForm.get('mode_input').valueChanges.subscribe(mode => {

      if (mode === 'TANPA') {

        // reset proforma
        this.entryForm.patchValue({ proforma_id: null });

        // enable semua input
        this.entryForm.get('customer_id').enable();
        this.entryForm.get('sls_kontrak_id').enable();
        this.entryForm.get('qty').enable();
        this.entryForm.get('harga_satuan').enable();
  ...
  hitungTotal() {
    ...
    if (ppn == 12) {
      nilai_ppn = (ppn / 100) * ((11 / 12) * dpp);
    } else {
      nilai_ppn = (ppn / 100) * dpp;
    }
  ...
  proformaChange(event) {
    console.log('proforma changed:', event);

    const id = event.id; // TANPA optional chaining

    const d = this.proformaList.find(x => String(x.id) === String(id));
    if (!d) {
      console.log('DATA TIDAK KETEMU');
      return;
    }

    console.log('DATA KETEMU:', d);
    // 🔥 cari lokasi object
    const lokasiObj = this.dataSelectLokasi.find(x => String(x.id) === String(d.lokasi_id));

    // 🔥 cari jenis invoice object
    const jenisObj = this.dataSelectJenisInvoice.find(x => x.id === d.jenis_performa);

    const customerObj = this.dataSelectCustomer.find(x => String(x.id) === String(d.customer_id));

    this.entryForm.patchValue({
      customer_id: customerObj
    });

    this.slsKontrakService.getAllbyCustomer(d.customer_id).subscribe(res => {

      this.dataSelectKontrak = [];
      this.kontrak = [];

      res['data'].forEach(k => {
        this.dataSelectKontrak.push({
          id: k.id,
          text: k.no_spk
        });
        this.kontrak.push(k);
      ...
      const kontrakObj = this.dataSelectKontrak.find(x => String(x.id) === String(d.sls_kontrak_id));

      this.entryForm.patchValue({
        lokasi_id: lokasiObj,
        jenis_invoice: jenisObj,
        sls_kontrak_id: kontrakObj,
        no_referensi: d.no_performa,
        deskripsi: d.deskripsi,

        qty: formatNumber(+d.qty, 'en_US', '1.2-2'),
        qty_real: formatNumber(+(d.qty_real || d.qty), 'en_US', '1.2-2'),
        harga_satuan: formatNumber(+d.harga_satuan, 'en_US', '1.2-2'),
        jumlah: formatNumber(+d.jumlah, 'en_US', '1.2-2'),

        diskon: formatNumber(+d.diskon || 0, 'en_US', '1.2-2'),
        uang_muka: formatNumber(+d.uang_muka || 0, 'en_US', '1.2-2'),
        premi: formatNumber(+d.premi || 0, 'en_US', '1.2-2'),
        ppn: +d.ppn || 0
      });

      this.entryForm.get('customer_id').disable();
      this.entryForm.get('sls_kontrak_id').disable();
      this.entryForm.get('qty').disable();
      this.entryForm.get('harga_satuan').disable();
