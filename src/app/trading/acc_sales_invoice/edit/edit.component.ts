...
export class EditComponent implements OnInit, AfterViewInit {
  ...
  constructor(private builder: FormBuilder,
    ...
    this.entryForm = this.builder.group({
      ...
      mode_input: new FormControl('', Validators.required),
  ...
  ngAfterViewInit(): void {
    ...
    if (this.accSalesInvoice && this.accSalesInvoice.proforma_id) {
      this.entryForm.patchValue({ mode_input: 'PROFORMA' });
    } else {
      this.entryForm.patchValue({ mode_input: 'TANPA' });
    }
  ...
  private loadSelect2(): void {
    ...
    this.slsPerformaService.getPerforma().subscribe(res => {
      ...
      // 🔥 PATCH PROFORMA LAMA (kalau ada)
      if (this.accSalesInvoice && this.accSalesInvoice.proforma_id) {
        const selected = this.dataSelectProforma.find(x => x.id == this.accSalesInvoice.proforma_id);
        if (selected) {
          this.entryForm.patchValue({ proforma_id: selected });
        }
      }
    });
  }
  ...
  proformaChange(event) {
    ...
    this.entryForm.patchValue({
      ...
      qty: d.qty,
      qty_real: d.qty_real || d.qty,
      harga_satuan: d.harga_satuan,
      jumlah: d.jumlah,

      diskon: d.diskon || 0,
      uang_muka: d.uang_muka || 0,
      premi: d.premi || 0,
      ppn: d.ppn || 0
    });
  ...
  isQtyRealReadonly(): boolean {

    const jenis = this.entryForm.get('jenis_invoice').value;

    if (!jenis) {
      return true;
    }

    const value = jenis.id ? jenis.id : jenis;

    return !['UANG MUKA', 'PENGGANTIAN', 'TERMIN'].includes(value);
  }
}
