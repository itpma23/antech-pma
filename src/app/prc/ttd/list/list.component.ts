...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/PrcPoTtd/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.prcPoTTD = resp.data;
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.prcPoTTDService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
  ...
  edit(id: number) {
    let that = this;
    let prcPoTTD;
    this.prcPoTTDService.getById(id).subscribe(data => {
      prcPoTTD = data['data'];
