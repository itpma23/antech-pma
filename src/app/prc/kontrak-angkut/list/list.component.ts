...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/PrcKontrakAngkut/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.prcKontrakAngkut = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.prcKontrakAngkut.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.prcKontrakAngkutService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
  ...
  edit(id: number) {
    let that = this;
    let prcKontrakAngkut;
    this.prcKontrakAngkutService.getById(id).subscribe(data => {
      prcKontrakAngkut = data['data'];
