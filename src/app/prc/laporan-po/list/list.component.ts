...
export class ListComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private authenticationService: AuthenticationService,
    ...
    this.laporanRincianPo = this.builder.group({
      ...
      jenis_laporan: new FormControl('detail', Validators.required),
    });
  ...
  reportPoRincian(tipe_laporan) {
    ...
    let supplier_id = (this.laporanRincianPo.controls['supplier'].value != null) ? this.laporanRincianPo.controls['supplier'].value['id'] : null;
    let format_laporan=this.laporanRincianPo.controls['format_laporan'].value;
    let jenis_laporan=this.laporanRincianPo.controls['jenis_laporan'].value;
    let data = {
      lokasi_id: lokasi_id,
      supplier_id: supplier_id,
      tgl_mulai: formatDate(this.laporanRincianPo.controls['tanggal_mulai'].value, "yyyy-MM-dd", 'en_US'),
      tgl_akhir: formatDate(this.laporanRincianPo.controls['tanggal_akhir'].value, "yyyy-MM-dd", 'en_US'),
      format_laporan: format_laporan,
      jenis_laporan: jenis_laporan,
    };

    console.log(data);
    this.prcPoService.getPoReportDetail(data).subscribe((res: any) => {
      if (format_laporan == 'xls') {
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
        saveAs(blob, 'rincian_po.xls')
