...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/GbmUom/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.gbmUom = resp.data;
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.gbmUomService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
  ...
  edit(id: number) {
    let that = this;
    let gbmUom;
    this.gbmUomService.getById(id).subscribe(data => {
      gbmUom = data['data'];
