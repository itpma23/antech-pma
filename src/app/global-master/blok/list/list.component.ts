...
export class ListComponent implements OnInit {
  ...
  checkuncheckall() {
    ...
    for (var i = 0; i < this.GbmBlok.length; i++) {
      this.GbmBlok[i].isSelected = this.isChecked;
    }
  ...
  isAllSelected() {
    this.isChecked = this.GbmBlok.every(function (item: any) {
  ...
  getCheckedItemList() {
    this.checkedList = [];
    for (var i = 0; i < this.GbmBlok.length; i++) {
      if (this.GbmBlok[i].isSelected)
        this.checkedList.push(this.GbmBlok[i]['id']);
    }
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/GbmBlok/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.GbmBlok = resp.data;
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.GbmBlokService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
  ...
  edit(id: number) {
    let that = this;
    let GbmBlok;
    this.GbmBlokService.getById(id).subscribe(data => {
      GbmBlok = data['data'];
  ...
  viewlaporan() {
    this.GbmBlokService.laporan_area().subscribe(
      (res) => {
        // // console.log(res);
  ...
  updateStatus() {
    ...
    swal({
      ...
      that.GbmBlokService.updateStatus(that.status_update, that.checkedList).subscribe(data => {
