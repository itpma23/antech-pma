...
export class ListComponent implements OnInit {
  ...
  checkuncheckall() {
    ...
    for (var i = 0; i < this.gbmSupplier.length; i++) {
      this.gbmSupplier[i].isSelected = this.isChecked;
    }
  ...
  isAllSelected() {
    this.isChecked = this.gbmSupplier.every(function (item: any) {
  ...
  getCheckedItemList() {
    this.checkedList = [];
    for (var i = 0; i < this.gbmSupplier.length; i++) {
      if (this.gbmSupplier[i].isSelected)
        this.checkedList.push(this.gbmSupplier[i]['id']);
    }
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/GbmSupplier/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.gbmSupplier = resp.data;
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.gbmSupplierService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
  ...
  edit(id: number) {
    let that = this;
    let gbmSupplier;
    this.gbmSupplierService.getById(id).subscribe(data => {
      gbmSupplier = data['data'];
  ...
  updateStatus() {
    ...
    swal({
      ...
      that.gbmSupplierService.updateStatus(that.status_update, that.checkedList).subscribe(data => {
