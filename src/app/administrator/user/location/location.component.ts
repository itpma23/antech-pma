...
export class LocationComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.userAccesService.getLocationAccess(this.user.id).subscribe(u => {
  ...
  private loadSelect2(): void {
    this.organisasiService.getAllAdmUnit().subscribe(x => {
      ...
      let selectOrganisasi = this.userAccesLocation['data'].map(x => {
        ...
        return ({ "id": x.location_id, "text": x.nama });
      });
      x.forEach(d => {
  ...
  onSubmit() {
    ...
    this.userAccesService.updateLocation(this.user.id, dataSubmit).subscribe(data => {
      console.log(data);
      if (data['status'] == 'OK') {
        console.log('ok');
        swal({
