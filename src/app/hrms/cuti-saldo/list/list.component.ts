  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/HrmsCutiSaldo/list', dataTablesParameters, {})
          .subscribe(resp => {
            this.hrmsCutiSaldo = resp.data;
...
return nama;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.hrmsCutiSaldo.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.hrmsCutiSaldoService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let hrmsCutiSaldo;
    this.hrmsCutiSaldoService.getById(id).subscribe(data => {
      hrmsCutiSaldo = data['data'];
