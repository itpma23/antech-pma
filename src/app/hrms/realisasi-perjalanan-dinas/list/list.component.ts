...
declare var $: any;
const MenuName = 'hrms_realisasi_perjalanan_dinas';
@Component({
  moduleId: module.id,
  selector: 'list-cmp',
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/HrmsRealisasiPerjalananDinas/list', dataTablesParameters, {})
          .subscribe(resp => {

            this.hrmsRealisasiPerjalananDinas = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.hrmsRealisasiPerjalananDinas.length > 0) {
  ...
  viewSlip(id) {
    ...
    this.hrmsRealisasiPerjalananDinasService.getPdfSlip(id).subscribe(
      (res) => {
        // // console.log(res);
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.hrmsRealisasiPerjalananDinasService.delete(id).subscribe(data => {
  ...
  edit(id: number) {
    let that = this;
    let hrmsRealisasiPerjalananDinas;
    this.hrmsRealisasiPerjalananDinasService.getById(id).subscribe(data => {
      hrmsRealisasiPerjalananDinas = data['data'];
  ...
  posting(id: number) {
    ...
    swal({
      ...
      that.hrmsRealisasiPerjalananDinasService.posting(id, data).subscribe(data => {
