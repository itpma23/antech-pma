...
declare var $: any;
const MenuName='hrms_tipe_karyawan';
@Component({
  moduleId: module.id,
  selector: 'list-cmp',
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/hrmsTipekaryawan/list', dataTablesParameters, {})
          .subscribe(resp => {
            this.tipeKaryawan = resp.data;
...
return nama;
  ...
  exportFiles(type) {
    ...
    this.exportAsService.save(this.exportAsConfig, 'tipeKaryawan').subscribe(() => { });
  }
  add() {
    let modalConfig = {
      animated: true,
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.hrmsTipeKaryawanService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let tipeKaryawan;
    this.hrmsTipeKaryawanService.getById(id).subscribe(data => {
      tipeKaryawan = data['data'];
  ...
  detail(id: number) {
    this.router.navigate(['tipeKaryawan/detail', id.toString(),{previousUrl: this.router.url}]);
