...
const MenuName = 'trk_kegiatan_kendaraan';
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        let kendaraan_id;
        if (isNullOrUndefined(this.parameterForm.get('kendaraan').value) != true) {
          if (isNullOrUndefined(this.parameterForm.get('kendaraan').value!.id)) {
            kendaraan_id = null
          } else {
            kendaraan_id = this.parameterForm.get('kendaraan').value.id;
          }
        } else {
          kendaraan_id = null
        }
        ...
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/trkKegiatanKendaraan/list', dataTablesParameters, {})
          .subscribe(resp => {

            this.trkKegiatanKendaraan = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.trkKegiatanKendaraan.length > 0) {
  ...
  exportExcel() {
    ...
    this.trkKegiatanKendaraanService.listTrkLogKendaraan(parameter).subscribe((res: any) => {
      if (format_laporan == 'xls') {
        console.log(res);
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
        saveAs(blob, 'list_logKendaraan.xls')
      } else {
  ...
  ListDetail() {
    ...
    console.log(parameter);
    this.trkKegiatanKendaraanService.listTrkKegiatanDetail(parameter).subscribe((res: any) => {
      if (format_laporan == 'xls') {
        console.log(res);
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.trkKegiatanKendaraanService.delete(id).subscribe(data => {
        swal({
          title: 'Deleted!',
  ...
  edit(id: number) {
    let that = this;
    let trkKegiatanKendaraan;
    this.trkKegiatanKendaraanService.getById(id).subscribe(data => {
      trkKegiatanKendaraan = data['data'];
  ...
  posting(id: number) {
    ...
    swal({
      ...
      that.trkKegiatanKendaraanService.posting(id, data).subscribe(data => {
  ...
  viewSlip(id) {
    ...
    this.trkKegiatanKendaraanService.getPdfSlip(id).subscribe(
      (res) => {
        // console.log(res);
        var fileURL = URL.createObjectURL(res);
        window.open(fileURL);
