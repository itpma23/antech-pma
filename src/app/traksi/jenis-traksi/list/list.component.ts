...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/TrkJenisTraksi/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.trkJenisTraksi = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.trkJenisTraksi.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.trkJenisTraksiService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
  ...
  edit(id: number) {
    let that = this;
    let trkJenisTraksi;
    this.trkJenisTraksiService.getById(id).subscribe(data => {
      trkJenisTraksi = data['data'];
