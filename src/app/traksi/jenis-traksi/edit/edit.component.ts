...
import { TrkJenisTraksi } from 'src/app/shared/models/trk_jenis_traksi.model';
...
export class EditComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    console.log(this.trkJenisTraksi);

    // this.entryForm.get('tanggal_efektif').patchValue(new Date(Date.parse(this.trkJenisTraksi.tanggal_efektif)));
    this.entryForm.controls['kode'].patchValue(this.trkJenisTraksi.kode);
    this.entryForm.controls['nama'].patchValue(this.trkJenisTraksi.nama);
  ...
  private loadSelect2(): void {
    ...
    //     if (a.id == this.trkJenisTraksi.lokasi_id) {
    //       selectMill = a;
    //     }
    ...
    //     if (a.id == this.trkJenisTraksi.supplier_id) {
    //       selectSupplier = a;
    //     }
  ...
  onSubmit() {
    ...
    this.trkJenisTraksiService.update(this.trkJenisTraksi.id, dataSubmit).subscribe(data => {
