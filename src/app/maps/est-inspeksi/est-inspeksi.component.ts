...
import { ViewInspeksiComponent } from './view/view-inspeksi.component';
...
@Component({
  moduleId: module.id,
  selector: 'inspeksi-maps-cmp',
  templateUrl: './est-inspeksi.component.html',
  styleUrls: ['./est-inspeksi.component.css']
})

export class EstInspeksiMapComponent implements OnInit {
  ...
  markerClicked($event: any, index: number) {
    console.log(index);
    console.log($event.latlng.lat, $event.latlng.lng);
    console.log(this.inspeksi[index]);
    this.view(index);
  ...
  constructor(private http: HttpClient, private authenticationService: AuthenticationService,
    ...
    private builder: FormBuilder, private estInspeksiService: EstInspeksiService
  ) {
  ...
  showData() {
    ...
    this.estInspeksiService.getInspeksiByTanggal(tanggal1, tanggal2).subscribe(
      d => {
        this.inspeksi = d['data'];
        console.log(this.inspeksi);

        for (let index = 0; index < this.inspeksi.length; index++) {

          const data = {
            position: { lat: this.inspeksi[index].lat, lng: this.inspeksi[index].lng },
            draggable: false,
            iconUrl:this.inspeksi[index].status=='1'?"../../assets/img/marker-blue.png":"../../assets/img/marker-red.png",
            iconShadowUrl:"../../assets/img/marker-shadow.png",
          };
          const marker = this.generateMarker(data, index);
          let html = "<b>Catatan:</b>" + this.inspeksi[index].catatan + "<br>";
          // html = html + "<img src='"+ this.PATH_URL+"/"+this.pathName+"/userfiles/inspeksi/"+  this.inspeksi[index].file+"' height='550px' width='650px' >";
  ...
  view(id: number) {
    ...
    this.bsModalRef = this.bsModalService.show(ViewInspeksiComponent, modalConfig);
