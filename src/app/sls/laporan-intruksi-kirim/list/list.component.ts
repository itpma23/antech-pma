...
export class ListComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private authenticationService: AuthenticationService,
    ...
    this.laporanMonitoring = this.builder.group({
      customer: new FormControl([]),
      // lokasi: new FormControl([]),
       tanggal_mulai: new FormControl(toDate, Validators.required),
      tanggal_akhir: new FormControl(toDate, Validators.required),
  ...
  reportRincian() {
    ...
    let customer_id = (this.laporanRincian.controls['customer'].value != null) ? this.laporanRincian.controls['customer'].value['id'] : null;
    let format_laporan = this.laporanRincian.controls['format_laporan'].value;
    let data = {
      no_ip: this.laporanRincian.controls['no_ip'].value,
      no_kontrak: this.laporanRincian.controls['no_kontrak'].value,
    ...
    console.log(data);
    this.SlsIntruksiService.getReportDetail(data).subscribe((res: any) => {
  ...
  reportMonitoring() {
    this.isFormRincianSubmitted = true;
    if (this.laporanMonitoring.invalid) {
      return;
    }

    // let lokasi_id = (this.laporanRincian.controls['lokasi'].value != null) ? this.laporanRincian.controls['lokasi'].value['id'] : null;
    let customer_id = (this.laporanMonitoring.controls['customer'].value != null) ? this.laporanMonitoring.controls['customer'].value['id'] : null;
    let format_laporan = this.laporanMonitoring.controls['format_laporan'].value;
    let status = this.laporanMonitoring.controls['status'].value;
    let data = {
      status: status,
      customer_id: customer_id,
      tgl_mulai: formatDate(this.laporanMonitoring.controls['tanggal_mulai'].value, "yyyy-MM-dd", 'en_US'),
      tgl_akhir: formatDate(this.laporanMonitoring.controls['tanggal_akhir'].value, "yyyy-MM-dd", 'en_US'),
      format_laporan: format_laporan,
    };

    console.log(data);
    this.SlsIntruksiService.getReportMonitoring(data).subscribe((res: any) => {
      if (format_laporan == 'xls') {
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
        saveAs(blob, 'rincian_kontrak.xls')
      } else {
