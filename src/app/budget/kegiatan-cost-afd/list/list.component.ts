...
const MenuName = 'bgt_kegiatan_cost';
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/BgtKegiatanCost/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.bgtKegiatanCost = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.bgtKegiatanCost.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.bgtKegiatanCostService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let bgtKegiatanCost;
    this.bgtKegiatanCostService.getById(id).subscribe(data => {
      bgtKegiatanCost = data['data'];
