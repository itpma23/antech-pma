...
customer_id;
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
        .post<DataTablesResponse>(this.apiUrl + '/SlsRekap/listRekapforInvoice/'+this.customer_id, dataTablesParameters, {})
        .subscribe(resp => {
  ...
  addItem(item) {
    ...
    // let i = this.SlsRekap.indexOf(item);

    // if (i != -1) {
    //   this.SlsRekap.splice(i, 1);
    //   this.event.emit(item);
  ...
  exportFiles(type) {
    ...
    this.exportAsService.save(this.exportAsConfig, 'SlsRekap').subscribe(() => { });
  }
