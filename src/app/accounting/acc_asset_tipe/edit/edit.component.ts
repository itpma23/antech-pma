...
import { AccAssetTipe } from 'src/app/shared/models/acc_asset_tipe.model';
...
export class EditComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    console.log(this.accAssetTipe);

    // this.entryForm.get('tanggal_efektif').patchValue(new Date(Date.parse(this.accAssetTipe.tanggal_efektif)));
    this.entryForm.controls['nama'].patchValue(this.accAssetTipe.nama);
  ...
  private loadSelect2(): void {
    ...
    //     if (a.id == this.accAssetTipe.lokasi_id) {
    //       selectMill = a;
    //     }
    ...
    //     if (a.id == this.accAssetTipe.supplier_id) {
    //       selectSupplier = a;
    //     }
    ...
    this.accAssetTipeService.getAll().subscribe(x => {
      this.dataSelectParent = [];
      x['data'].forEach(d => {
        this.dataSelectParent.push({ "id": d.id, "text": d.nama });
        if (d.id == this.accAssetTipe.parent_id) {
          selectParent = d;
        }
        this.entryForm.controls['parent_id'].patchValue(selectParent);
      });
    });
  ...
  onSubmit() {
    ...
    this.accAssetTipeService.update(this.accAssetTipe.id, dataSubmit).subscribe(data => {
