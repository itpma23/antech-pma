...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      columns: [
        ...
        { 'data': 'nama', 'width': "15%"},
        { 'data': 'uom', 'width': "7%"},
        { 'data': 'kegiatan_kelompok' ,'width': "10%"},
        { 'data': 'tipe_kegiatan','width': "10%" },
        { 'data': 'acc_akun','width': "15%" },
        {'data': 'users', 'width': "22%",}
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/AccKegiatan/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.accKegiatan = resp.data;
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.accKegiatanService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let accKegiatan;
    this.accKegiatanService.getById(id).subscribe(data => {
      accKegiatan = data['data'];
