...
const MenuName = 'acc_kegiatan_kelompok';
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/AccKegiatanKelompok/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.accKegiatanKelompok = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.accKegiatanKelompok.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.accKegiatanKelompokService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let accKegiatanKelompok;
    this.accKegiatanKelompokService.getById(id).subscribe(data => {
      accKegiatanKelompok = data['data'];
