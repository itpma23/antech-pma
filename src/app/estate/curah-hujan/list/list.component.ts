...
const MenuName = 'est_curah_hujan';
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/EstCurahHujan/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.estCurahHujan = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.estCurahHujan.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.estCurahHujanService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let estCurahHujan;
    this.estCurahHujanService.getById(id).subscribe(data => {
      estCurahHujan = data['data'];
