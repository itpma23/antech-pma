...
const MenuName = 'est_potongan_karyawan';
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/EstPotonganKaryawan/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.estPotonganKaryawan = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.estPotonganKaryawan.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.estPotonganKaryawanService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
        swal({
  ...
  edit(id: number) {
    let that = this;
    let estPotonganKaryawan;
    this.estPotonganKaryawanService.getById(id).subscribe(data => {
      estPotonganKaryawan = data['data'];
