...
export class EditComponent implements OnInit, AfterViewInit {
        ...
        this.gbmOrganisasiService.getBlokByEstate(this.bkmPemeliharaan.lokasi_id).subscribe(x => {
          ...
          x.forEach(d => {
            this.dataSelectBlok.push({ "id": d.id, "text": d.kode + " - " + d.nama });
                    ...
                    let blokPertama = this.bkmPemeliharaan.detail[0].blok_id;

          let dataBlok = this.dataBlok.find(x => x.id == blokPertama);

          let statusblok = '';

          if (dataBlok) {
            statusblok = dataBlok.statusblok;
          }

          console.log('STATUS BLOK EDIT', statusblok);
          this.accKegiatanService.getAllByStatusBlokTipe(
            'PEMELIHARAAN',
            statusblok
          ).subscribe(k => {
  ...
  onSubmit() {
    ...
    console.log(frmData);
    this.estBkmPemeliharaanService.update(this.bkmPemeliharaan.id, frmData).subscribe(data => {
      // console.log(data);
      if (data['status'] == 'OK') {
        // console.log('ok');
        this.event.emit('OK');
  ...
  recalculateHKPremi() {
    let id = this.bkmPemeliharaan.id;

    this.estBkmPemeliharaanService.calculateHk(id).subscribe(res => {
        ...
        for (let i = 0; i < details.length; i++) {
          ...
          if (row) {
            if (row.get('rupiah_hk')) {
              row.get('rupiah_hk').patchValue(details[i].rupiah_hk);
            }
    ...
    this.loadKegiatanByStatusBlok(blokId);

  }

  loadKegiatanByStatusBlok(blokId) {
    ...
    console.log('Blok:', dataBlok.nama);
    console.log('Status:', dataBlok.statusblok);

    this.accKegiatanService
      .getAllByStatusBlokTipe('PEMELIHARAAN', dataBlok.statusblok)
      .subscribe(res => {
