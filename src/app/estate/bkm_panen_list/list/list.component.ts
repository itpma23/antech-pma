...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/estBkmPanen/list_pemanen', dataTablesParameters, {})
          .subscribe(resp => {

            this.estBkmPanen = resp.data;
            // console.log(this.estBkmPanen);
            callback({
  ...
  ListDetail() {
    ...
    console.log(parameter);
    this.estBkmPanenService.listBkmPanenDetail(parameter).subscribe((res: any) => {
      if (format_laporan == 'xls') {
        console.log(res);
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
        saveAs(blob, 'list_panen.xls')
      } else {
  ...
  exportExcel() {
    ...
    this.estBkmPanenService.listBkmPanen(parameter).subscribe((res: any) => {
      if (format_laporan == 'xls') {
        console.log(res);
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
        saveAs(blob, 'list_bkm.xls')
      } else {
        var fileURL = URL.createObjectURL(res);
        window.open(fileURL);
      }
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.estBkmPanenService.delete(id).subscribe(data => {
  ...
  edit(id: number) {
    let that = this;
    let estBkmPanen;
    this.estBkmPanenService.getById(id).subscribe(data => {
      estBkmPanen = data['data'];
  ...
  posting(id: number) {
    ...
    swal({
      ...
      that.estBkmPanenService.posting(id, data).subscribe(data => {
        if (data['status'] == 'OK') {
          that.rerender();
          swal({
            title: 'Info!',
  ...
  viewSlip(id) {
    ...
    this.estBkmPanenService.getPdfSlip(id).subscribe(
      (res) => {
        // console.log(res);
        var fileURL = URL.createObjectURL(res);
        window.open(fileURL);
