...
const MenuName = 'est_param_premi_mandor_kerani';
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/EstParamPremiMandorKerani/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.estParamPremiMandorKerani = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.estParamPremiMandorKerani.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.estParamPremiMandorKeraniService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
  ...
  edit(id: number) {
    let that = this;
    let estParamPremiMandorKerani;
    this.estParamPremiMandorKeraniService.getById(id).subscribe(data => {
      estParamPremiMandorKerani = data['data'];
