...
const MenuName = 'est_produksi_panen';
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/estProduksipanen/list', dataTablesParameters, {})
          .subscribe(resp => {
            this.produksi_panen = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.produksi_panen.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.EstProduksiPanenService.delete(id).subscribe(data => {
  ...
  edit(id: number) {
    let that = this;
    let estRekapPanen;
    this.EstProduksiPanenService.getById(id).subscribe(data => {
      estRekapPanen = data['data'];
  ...
  viewSlip(id) {
    ...
    this.EstProduksiPanenService.getPdfSlip(id).subscribe(
      (res) => {
        // console.log(res);
