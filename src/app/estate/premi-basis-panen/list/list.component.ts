...
const MenuName = 'est_premi_basis_panen';
...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      columns: [
        ...
        {'data': 'premi_basis', },
        {'data': 'lebih_basis1', },
        {'data': 'premi_lebih_basis1', },
        {'data': 'users', 'width': "20%",},
        // {'data': 'premi_lebih_basis2', },
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/EstPremiBasisPanen/list/' + this.status, dataTablesParameters, {})
          .subscribe(resp => {
            this.estPremiBasisPanen = resp.data;
  ...
  rerender(): void {
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      setTimeout(() => {
        ...
        if (this.estPremiBasisPanen.length > 0) {
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.estPremiBasisPanenService.delete(id).subscribe(data => {
        // let t = $('#datatables').DataTable().ajax.reload();
        // t.draw();
        that.rerender();
  ...
  edit(id: number) {
    let that = this;
    let estPremiBasisPanen;
    this.estPremiBasisPanenService.getById(id).subscribe(data => {
      estPremiBasisPanen = data['data'];
