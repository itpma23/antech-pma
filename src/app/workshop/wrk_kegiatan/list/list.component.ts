...
export class ListComponent implements OnInit {
  ...
  loadDatatable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.http
          .post<DataTablesResponse>(this.apiUrl + '/WrkKegiatan/list', dataTablesParameters, {})
          .subscribe(resp => {

            this.WrkKegiatan = resp.data;
  ...
  exportExcel() {
    ...
    this.WrkKegiatanService.listWrkKegiatna(parameter).subscribe((res: any) => {
      if (format_laporan == 'xls') {
        console.log(res);
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
        saveAs(blob, 'list_workshop.xls')
      } else {
  ...
  ListDetail() {
    ...
    console.log(parameter);
    this.WrkKegiatanService.listWrkKegiatanDetail(parameter).subscribe((res: any) => {
      if (format_laporan == 'xls') {
        console.log(res);
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
  ...
  delete(id: number) {
    ...
    swal({
      ...
      that.WrkKegiatanService.delete(id).subscribe(data => {
         swal({
        title: 'Deleted!',
  ...
  edit(id: number) {
    let that = this;
    let WrkKegiatan;
    this.WrkKegiatanService.getById(id).subscribe(data => {
      WrkKegiatan = data['data'];
  ...
  posting(id: number) {
    ...
    swal({
      ...
      that.WrkKegiatanService.posting(id, data).subscribe(data => {
        if (data['status'] == 'OK') {
          that.rerender();
          swal({
            title: 'Info!',
  ...
  viewSlip(id) {
    ...
    this.WrkKegiatanService.getPdfSlip(id).subscribe(
      (res) => {
        // console.log(res);
        var fileURL = URL.createObjectURL(res);
        window.open(fileURL);
