...
export class ListComponent implements OnInit {
  ...
  private loadSelect2(): void {
    ...
    this.GbmOrganisasiService.getAllByType("UNIT").subscribe(x => {
      ...
      // this.laporanRincian.controls['lokasi'].valueChanges.subscribe(x => {
      //   let org_id = x.id;
      //   console.log(x)
  ...
  reportPoRincian(tipe_laporan) {
    ...
    // let gudang_id = (this.laporanRincian.controls['gudang'].value != null) ? this.laporanRincian.controls['gudang'].value['id'] : null;
    ...
    console.log(data);
    this.InvReturPemakaianBarangService.getPoReportDetail(data).subscribe((res: any) => {
      if (format_laporan == 'xls') {
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
        saveAs(blob, 'laporan_rincian_retur_pemakaian_barang.xls')
