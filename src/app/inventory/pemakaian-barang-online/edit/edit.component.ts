...
constructor(private builder: FormBuilder,
  ...
  ngAfterViewInit(): void {
    ...
    this.entryForm.get('catatan').patchValue(this.invPemakaianBarang.catatan);
    this.entryForm.get('no_transaksi').patchValue(this.invPemakaianBarang.no_transaksi);
  ...
  private loadSelect2(): void {
    ...
    this.dataSelectTipe.forEach(a => {
      if (a.id == this.invPemakaianBarang.tipe) {
        selectTipe = a;
      }
    });
    ...
    this.gbmOrganisasiService.getAllByType('AFDELING_STASIUN_WORKSHOP').subscribe(x=>{
      ...
      x.forEach(d => {
        ...
        if (this.invPemakaianBarang.lokasi_afd_id == d.id) {
    ...
    this.gbmOrganisasiService.getAllByType('TRAKSI').subscribe(x=>{
      ...
      x.forEach(d => {
        ...
        if (this.invPemakaianBarang.lokasi_traksi_id == d.id) {
    ...
    this.gbmOrganisasiService.getAllGudangCentralAndVirtual().subscribe(x=>{
      ...
      x.forEach(d => {
        ...
        if (this.invPemakaianBarang.gudang_id == d.id) {
    ...
    this.karyawanService.getAll().subscribe(x=>{
      ...
      x['data'].forEach(d => {
        ...
        if (this.invPemakaianBarang.karyawan_id == d.id) {
  ...
  onSubmit() {
    ...
    // // console.log(frmData);
    this.invPemakaianBarangService.update(this.invPemakaianBarang.id,frmData).subscribe(data => {
      // console.log(data);
      if( data['status']=='OK'){
        console.log('ok');
        swal({
