...
export class DetailComponent implements OnInit, AfterViewInit {
  ...
  onSubmit() {
    ...
    if (this.formUploadFile.invalid) {
      return;
    }

    const file: File = this.formUploadFile.get('file').value;
        ...
        this.invPengeluaranBarangService.proses(this.invPengeluaranBarang.id, {}).subscribe(postingResponse => {
          if (postingResponse['status'] === 'OK') {
            ...
            if (Array.isArray(items)) {
              ...
              swal({
                title: 'Info!',
                text: 'Ada Stok Minus.\n' + msg + '\nSilahkan Melakukan Reject Approval',
                type: 'warning',
                confirmButtonClass: "btn btn-success",
                buttonsStyling: false
              });
              ...
              swal({
                title: 'Info!',
                text: 'Posting gagal: ' + postingResponse['data'] + '\nSilahkan Melakukan Reject Approval',
                type: 'warning',
                confirmButtonClass: "btn btn-success",
                buttonsStyling: false
              });
  ...
  ngOnInit() {
    this.loadSelect2();
    this.loadImage(this.invPengeluaranBarang.id)
  ...
  loadImage(id: any): void {
    this.invPengeluaranBarangService.getEvidence(id).subscribe({
      next: (response) => {
        // Paksa casting ke Blob
  ...
  get isFileEmpty(): boolean {
  const fileControl = this.formUploadFile.get('file');
  return !fileControl || fileControl.value == null || fileControl.value === '';
}
