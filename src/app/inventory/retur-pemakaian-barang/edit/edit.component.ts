...
import { InvReturPemakaianBarang } from 'src/app/shared/models/inv_retur_pemakaian_barang.model';
...
constructor(private builder: FormBuilder,
  ...
  ngAfterViewInit(): void {
    ...
    this.entryForm.get('tanggal').patchValue(new Date(Date.parse(this.invReturPemakaianBarang.tanggal)));
    this.entryForm.get('catatan').patchValue(this.invReturPemakaianBarang.catatan);
    this.entryForm.get('no_transaksi').patchValue(this.invReturPemakaianBarang.no_transaksi);
  ...
  private loadSelect2(): void {
    ...
    this.dataSelectTipe.forEach(a => {
      if (a.id == this.invReturPemakaianBarang.tipe) {
        selectTipe = a;
      }
    });
    ...
    this.gbmOrganisasiService.getAllByType('UNIT').subscribe(x=>{
      ...
      x.forEach(d => {
        ...
        if (this.invReturPemakaianBarang.lokasi_id == d.id) {
    ...
    this.gbmOrganisasiService.getAllByType('AFDELING_STASIUN').subscribe(x=>{
      ...
      x.forEach(d => {
        ...
        if (this.invReturPemakaianBarang.lokasi_afd_id == d.id) {
    ...
    this.gbmOrganisasiService.getAllByType('TRAKSI').subscribe(x=>{
      ...
      x.forEach(d => {
        ...
        if (this.invReturPemakaianBarang.lokasi_traksi_id == d.id) {
    ...
    this.gbmOrganisasiService.getAllByType('GUDANG').subscribe(x=>{
      ...
      x.forEach(d => {
        ...
        if (this.invReturPemakaianBarang.gudang_id == d.id) {
    ...
    this.karyawanService.getAll().subscribe(x=>{
      ...
      x['data'].forEach(d => {
        ...
        if (this.invReturPemakaianBarang.karyawan_id == d.id) {
    ...
    let selectPemakaian={};
    this.invPemakaianBarangService.getById(this.invReturPemakaianBarang.inv_pemakaian_id).subscribe(x=>{
     console.log(x)
     let d=x['data'];
      this.dataSelectPemakaian=[];
      // x['data'].forEach(d => {
        this.dataSelectPemakaian.push({"id":d['id'],"text":d['no_transaksi']});
        // if (this.invReturPemakaianBarang.inv_pemakaian_id == d.id) {
          selectPemakaian ={"id":d['id'],"text":d['no_transaksi']}
        // }
      // });
      this.entryForm.get('inv_pemakaian_id').patchValue(selectPemakaian);
    });
    ...
    this.invItemService.getAll().subscribe(x=>{
      ...
      this.gbmUomService.getAll().subscribe(x=>{
        ...
        this.invPemakaianBarangService.getTraksi().subscribe(x=>{
      ...
      this.gbmOrganisasiService.getAllByType('BLOK_MESIN').subscribe(x=>{
        ...
        this.accKegiatanService.getAllbyTipe('BAHAN').subscribe(x=>{
          ...
          let dtl = [];
          dtl = this.invReturPemakaianBarang.detail;
  ...
  onSubmit() {
    ...
    // // console.log(frmData);
    this.invReturPemakaianBarangService.update(this.invReturPemakaianBarang.id,frmData).subscribe(data => {
      // console.log(data);
      if( data['status']=='OK'){
        console.log('ok');
        swal({
